...
parameters
{
   ...
   real<lower=lambdap[3],upper=lambdap[4]> lambda[J,K];
   real<lower=deltap[3],upper=deltap[4]> delta[J,K];
...
transformed parameters
{
   ...
   for (k in 1:K) {
      for (j in 1:J) {
         ...
         for (i in 1:I+1) {
            mu[j,k,i] = inv_logit(bias[j,k,i]+logit(xall[i]));
...
model
{
   ...
   for (k in 1:K) {
      for (j in 1:J) {
         ...
         lambda[j,k] ~ uniform(lambdap[1],lambdap[2]);
         delta[j,k] ~ uniform(deltap[1],deltap[2]);
         #eta[j,k] ~ gamma(lambda[j,k],delta[j,k]);
         eta[j,k] ~ gamma(lambda[j,k]^2/delta[j,k],lambda[j,k]/delta[j,k]);
         for (i in 1:I) {
